...
class GENConv(GenMessagePassing):
    ...
    """
    def __init__(self, in_dim, emb_dim, aggr='softmax', t=1.0, learn_t=False,
                 p=1.0, learn_p=False, msg_norm=False, learn_msg_scale=True,
                 encode_edge=False, bond_encoder=False, edge_feat_dim=None,
                 norm='batch', mlp_layers=2, eps=1e-7):

        super(GENConv, self).__init__(aggr=aggr, t=t, learn_t=learn_t, p=p,
                                      learn_p=learn_p)
...
class MRConv(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        x_j = tg.utils.scatter_(
            ...
            torch.index_select(x, 0, edge_index[1]), edge_index[1],
            dim_size=x.shape[0])
...
class SAGEConv(tg.nn.SAGEConv):
    ...
    """
    def __init__(self, in_channels, out_channels, nn, norm=True, bias=True,
                 relative=False, **kwargs):
        self.relative = relative
        if norm is not None:
            super(SAGEConv, self).__init__(in_channels, out_channels, True,
                                           bias, **kwargs)
        else:
            super(SAGEConv, self).__init__(in_channels, out_channels, False,
                                           bias, **kwargs)
        self.nn = nn
...
class RSAGEConv(SAGEConv):
    ...
    """
    def __init__(self, in_channels, out_channels, act='relu', norm=None,
                 bias=True, relative=False):
        nn = MLP([out_channels + in_channels, out_channels], act, norm, bias)
        super(RSAGEConv, self).__init__(in_channels, out_channels, nn, norm,
                                        bias, relative)
...
class GinConv(tg.nn.GINConv):
    ...
    def __init__(self, in_channels, out_channels, act='relu', norm=None,
                 bias=True, aggr='add'):
        super(GinConv,
              self).__init__(MLP([in_channels, out_channels], act, norm, bias))
...
class GraphConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv='edge', act='relu',
        ...
        if conv.lower() == 'edge':
            ...
            self.gconv = GATConv(in_channels, out_channels // heads, act, norm,
                                 bias, heads)
        elif conv.lower() == 'gcn':
            self.gconv = SemiGCNConv(in_channels, out_channels, act, norm,
                                     bias)
        elif conv.lower() == 'gin':
            self.gconv = GinConv(in_channels, out_channels, act, norm, bias)
        elif conv.lower() == 'sage':
            self.gconv = RSAGEConv(in_channels, out_channels, act, norm, bias,
                                   False)
...
class DynConv(GraphConv):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=9, dilation=1,
                 conv='edge', act='relu', norm=None, bias=True, heads=8,
                 **kwargs):
...
class DenseDynBlock(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels=64, kernel_size=9, dilation=1,
                 conv='edge', act='relu', norm=None, bias=True, **kwargs):
        super(DenseDynBlock, self).__init__()
        self.body = DynConv(in_channels, out_channels, kernel_size, dilation,
...
class ResGraphBlock(nn.Module):
    ...
    """
    def __init__(self, channels, conv='edge', act='relu', norm=None, bias=True,
                 heads=8, res_scale=1):
...
class DenseGraphBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv='edge', act='relu',
        ...
        self.body = GraphConv(in_channels, out_channels, conv, act, norm, bias,
                              heads)
