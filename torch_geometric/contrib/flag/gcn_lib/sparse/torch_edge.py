...
def knn_matrix(x, k=16, batch=None):
    ...
    with torch.no_grad():
        ...
        n_points = x.shape[1]
        start_idx = torch.arange(0, n_points * batch_size,
                                 n_points).long().view(batch_size, 1, 1)
        if x.is_cuda:
        ...
        center_idx = torch.arange(0, n_points * batch_size).repeat(
            k, 1).transpose(1, 0).contiguous().view(1, -1)
        if x.is_cuda:
